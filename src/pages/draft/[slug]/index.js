  ...
  const buildEvent = async draft => {
    ...
    switch (draft?.type) {
      case 'document':
        ...
        event.tags = [
          ...
          ...(draft?.price
            ...
            : []),
          ...(draft?.additionalLinks
            ? draft.additionalLinks
                .filter(link => link !== 'https://plebdevs.com')
                .map(link => ['r', link])
            : []),
        ];
      ...
      case 'video':
        ...
        if (
          draft?.content.includes('.mp4') ||
          draft?.content.includes('.mov') ||
          draft?.content.includes('.avi') ||
          draft?.content.includes('.wmv') ||
          draft?.content.includes('.flv') ||
          draft?.content.includes('.webm')
        ) {
  ...
  return (
    <div className="w-full px-12 pt-12 mx-auto mt-4">
      <div className="w-full flex flex-row justify-between max-tab:flex-col max-mob:flex-col">
        <i
          className="pi pi-arrow-left pr-8 cursor-pointer hover:opacity-75"
          onClick={() => router.push('/')}
        />
        <div className="w-[75vw] mx-auto flex flex-row items-start justify-between">
          <div className="flex flex-col items-start max-w-[45vw] max-tab:max-w-[100vw] max-mob:max-w-[100vw]">
            <div className="pt-2 flex flex-row justify-start w-full">
              {draft?.topics &&
                draft.topics.map((topic, index) => {
                  if (topic === '') return;
                  return <Tag className="mr-2 text-white" key={index} value={topic}></Tag>;
                })}
            </div>
            ...
            {draft?.price && <p className="text-lg mt-4">Price: {draft.price}</p>}
          ...
          <div className="flex flex-col max-tab:mt-12 max-mob:mt-12">
            {draft && (
              <div
                style={{ width: width < 768 ? 'auto' : width }}
                onClick={() => router.push(`/details/${nAddress}`)}
                className="flex flex-col items-center mx-auto cursor-pointer rounded-md shadow-lg"
              >
                <div
                  style={{ maxWidth: width, minWidth: width }}
                  className="max-tab:h-auto max-mob:h-auto"
                >
                  <Image
                    alt="resource thumbnail"
      ...
      <div className="w-[75vw] mx-auto flex flex-row justify-end mt-12">
        <div className="w-fit flex flex-row justify-between">
          ...
          />
          <GenericButton
            onClick={() => router.push(`/draft/${draft?.id}/edit`)}
            label="Edit"
            severity="warning"
            outlined
            className="w-auto m-2"
