...
const CombinedContributionChart = ({ session }) => {
  ...
  const prepareProgressData = useCallback(() => {
    ...
    session.user.userCourses.forEach(courseProgress => {
      ...
      if (courseProgress.completed) {
        ...
        const date = new Date(
          completeDate.getTime() - completeDate.getTimezoneOffset() * 60000
        ).toLocaleDateString('en-CA');
        activityData[date] = (activityData[date] || 0) + 1;
        allActivities.push({
    ...
    session.user.userLessons?.forEach(lessonProgress => {
      if (lessonProgress.opened) {
        ...
        const date = new Date(
          openDate.getTime() - openDate.getTimezoneOffset() * 60000
        ).toLocaleDateString('en-CA');
        activityData[date] = (activityData[date] || 0) + 1;
        allActivities.push({
          type: 'lesson_started',
  ...
  const { data, isLoading, isFetching, error, refetch } = useFetchGithubCommits(
    session,
    handleNewCommit
  );
  ...
  useEffect(() => {
    if (data && !isLoading) {
      ...
      setTotalContributions(
        data.totalCommits + Object.values(activityData).reduce((a, b) => a + b, 0)
      );
    }
  ...
  const generateCalendar = useCallback(() => {
    ...
    while (currentDate <= today) {
      ...
      const activityCount =
        (contributionData[dateString] || 0) - (data?.contributionData[dateString] || 0);
        ...
        <div className="min-w-[910px] p-4">
          {(isLoading || isFetching) && (
            <h4 className="text-base font-semibold text-gray-200 mb-3 flex items-center gap-2">
              Loading contribution data... ({totalContributions} total contributions / activities
              fetched)
                        ...
                        row[weekIndex] && (
                          <div
                            ...
                            title={`${row[weekIndex].date.toDateString()}: ${
                              [
                                row[weekIndex].githubCount > 0
                                  ? `${row[weekIndex].githubCount} contribution${row[weekIndex].githubCount !== 1 ? 's' : ''}`
                                  : '',
                                row[weekIndex].activityCount > 0
                                  ? `${row[weekIndex].activityCount} activit${row[weekIndex].activityCount !== 1 ? 'ies' : 'y'}`
                                  : '',
                              ]
                                .filter(Boolean)
                                .join(' & ') || 'No contributions or activities'
                            }`}
                          ></div>
                        )
                    )}
