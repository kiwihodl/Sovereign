...
const LessonSelector = ({
  ...
  const updateContentOptions = () => {
    ...
    const filterContent = content => {
      const contentPrice =
        content?.price ||
        (content?.tags && content?.tags.find(tag => tag[0] === 'price')?.[1]) ||
        0;
    ...
    const draftDocumentOptions = filteredContent
      .filter(content => content?.topics.includes('document') && !content.kind)
      .map(content => ({
        label: content.title,
        value: content,
      }));

    const draftVideoOptions = filteredContent
      .filter(content => content?.topics.includes('video') && !content.kind)
      .map(content => ({
        label: content.title,
        value: content,
      }));

    const documentOptions = filteredContent
      .filter(content => content?.topics.includes('document') && content.kind)
      .map(content => ({
        label: content.title,
        value: content,
    ...
    const videoOptions = filteredContent
      .filter(content => content?.type === 'video' && content.kind)
      .map(content => ({
        label: content.title,
        value: content,
      }));

    const combinedOptions = filteredContent
      .filter(
        content =>
          content?.topics?.includes('video') &&
          content?.topics?.includes('document') &&
          content.kind
      )
      .map(content => ({
        label: content.title,
      ...
      <Accordion multiple activeIndex={openTabs} onTabChange={handleTabChange}>
        {lessons.map((lesson, index) => (
          <AccordionTab key={index} header={<AccordianHeader lesson={lesson} index={index} />}>
              ...
              <Dropdown
                ...
                itemTemplate={option => (
                  <ContentDropdownItem
                    ...
                    onSelect={content => handleContentSelect(content, index)}
                  />
                )}
            ...
            {lesson.id && (
              <div className="mt-4">
                <SelectedContentItem content={lesson} onRemove={() => handleRemoveContent(index)} />
              </div>
            )}
          </AccordionTab>
        ))}
