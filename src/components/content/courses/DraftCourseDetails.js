...
export default function DraftCourseDetails({ processedEvent, draftId, lessons }) {
  ...
  const handleDelete = () => {
    axios
      .delete(`/api/courses/drafts/${processedEvent.id}`)
      .then(() => {
    ...
    try {
      ...
      await Promise.all(
        createdLessons.map(lesson =>
          axios.put(`/api/lessons/${lesson.id}`, { courseId: newCourseId })
        )
      );
  ...
  const createCourseEvent = (courseId, title, summary, coverImage, lessons, price) => {
    ...
    event.tags = [
      ...
      ...lessons
        ...
        .map(lesson => ['a', `${lesson.kind}:${lesson.pubkey}:${lesson.d}`]),
    ];
    return event;
  };
    ...
    <div className="w-full px-24 pt-12 mx-auto mt-4 max-tab:px-0 max-mob:px-0 max-tab:pt-2 max-mob:pt-2">
        ...
        <div className="w-[75vw] mx-auto flex flex-row items-start justify-between max-tab:flex-col max-mob:flex-col max-tab:w-[95vw] max-mob:w-[95vw]">
          <div className="flex flex-col items-start max-w-[45vw] max-tab:max-w-[100vw] max-mob:max-w-[100vw]">
            ...
            <p className="pt-8 text-sm text-gray-400">
              {processedEvent?.createdAt && formatDateTime(processedEvent?.createdAt)}
            </p>
          </div>
      ...
      <div className="w-[75vw] mx-auto flex flex-row justify-end mt-12">
        <div className="w-fit flex flex-row justify-between">
          ...
          />
          <GenericButton
            onClick={() => router.push(`/course/${draftId}/draft/edit`)}
            label="Edit"
            severity="warning"
            outlined
            className="w-auto m-2"
