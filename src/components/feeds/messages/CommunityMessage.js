  ...
  return (
    <div className="flex flex-row justify-between items-end my-2">
      ...
      <GenericButton
        ...
        icon={options.collapsed ? 'pi pi-chevron-down' : 'pi pi-chevron-up'}
        tooltip={windowWidth <= 768 ? null : 'comments'}
        tooltipOptions={{ position: 'left' }}
      />
    </div>
  );
  ...
  const header = (
    <div className="flex flex-row w-full items-center justify-between p-4 bg-gray-800 rounded-t-lg">
      <div className="flex flex-row items-center">
        <Avatar
          image={message.avatar ? message.avatar : null}
          icon={message.avatar ? null : 'pi pi-user'}
          shape="circle"
          size="large"
      ...
      <div className="flex flex-col items-start justify-between">
        <div className="flex flex-row w-full justify-between items-center my-1 max-sidebar:flex-col max-sidebar:items-start">
          <Tag
            ...
            severity="primary"
            className="w-fit text-[#f8f8ff] bg-gray-600 mr-2 max-sidebar:mr-0"
          />
          <Tag
            icon={getPlatformIcon(platform)}
            value={platform}
            className="w-fit text-[#f8f8ff] bg-blue-400 max-sidebar:mt-1"
          />
        </div>
      </div>
    </div>
